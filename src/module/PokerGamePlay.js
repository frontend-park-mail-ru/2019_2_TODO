...
export class game {
  ...
  startRound() {
    ...
    sessionStorage.botBet = 0;
    updateScoreBet();
    const playersCards = this.getNoRandomHand(2);
    const botsCards = this.getNoRandomHand(2);
    const bankCards = this.getNoRandomHand(5);
      ...
      if (this._stage < 4) {
        setTimeout(() => {
          if (this.botHand.rank < 2) {
            ...
            evt.target.parentElement.id = 'botPanel';
            console.log(evt);
            this.raise(evt, parseInt(sessionStorage.playerBet) - parseInt(sessionStorage.botBet) + 40);
          }
        }, 1000);
      }
    });
